...
GitHub Pages, served under the repo subpath (`/imposter_game/`). **Deploy is user-run, not agent-run.**

1. `npm run build`
2. Publish `dist/` to the `gh-pages` branch (e.g. with `gh-pages -d dist` or any preferred tool).

Vite `base` is set to `/imposter_game/`; manifest and service-worker scopes resolve under that prefix.
